...
// Loads asil://welcome first to simulate a first run and then loads another
...
IN_PROC_BROWSER_TEST_F(BraveNetworkAuditTest, BasicTests) {
  ...
  ASSERT_TRUE(ui_test_utils::NavigateToURL(browser(), GURL("asil://welcome")));
  ...
  ASSERT_TRUE(ui_test_utils::NavigateToURL(browser(), GURL("asil://newtab")));
  WaitForTimeout(kMaxTimeoutPerLoadedURL);
  ...
  // Finally, load asil://rewards and enable Brave Rewards.
  ASSERT_TRUE(ui_test_utils::NavigateToURL(browser(), GURL("asil://rewards")));
  ASSERT_TRUE(EnableBraveRewards());
  WaitForTimeout(kMaxTimeoutPerLoadedURL);

  ASSERT_TRUE(ui_test_utils::NavigateToURL(browser(), GURL("asil://wallet")));
  WaitForTimeout(kMaxTimeoutPerLoadedURL);
