...
IN_PROC_BROWSER_TEST_F(BraveSchemeLoadBrowserTest, NotAllowedToLoadTest) {
  ...
  console_observer.SetPattern(
      "Not allowed to load local resource: asil://settings/");
...
IN_PROC_BROWSER_TEST_F(BraveSchemeLoadBrowserTest,
  ...
  ASSERT_TRUE(
      content::ExecJs(active_contents(), "window.open(\"asil://settings\")"));
  ASSERT_TRUE(console_observer.Wait());
}
...
// This test is for the asil:// scheme. This is a regression test added with:
...
IN_PROC_BROWSER_TEST_F(BraveSchemeLoadBrowserTest, MAYBE_CrashURLTest) {
  ...
  browser()->OpenURL(
      content::OpenURLParams(GURL("asil://crash/"), content::Referrer(),
...
// Ex, asil://settings should be loaded only in normal window because
// chrome://settings is not allowed. When tyring to loading asil://settings in
// private window, it should be loaded in normal window instead of private
// window.
IN_PROC_BROWSER_TEST_F(BraveSchemeLoadBrowserTest,
                       SettingsPageIsNotAllowedInPrivateWindow) {
  TestURLIsNotLoadedInPrivateWindow("asil://settings");
}
...
IN_PROC_BROWSER_TEST_F(BraveSchemeLoadBrowserTest,
  ...
  TestURLIsNotLoadedInPrivateWindow("asil://rewards");
}
...
IN_PROC_BROWSER_TEST_F(BraveSchemeLoadBrowserTest,
  ...
  TestURLIsNotLoadedInPrivateWindow("asil://wallet");
}

IN_PROC_BROWSER_TEST_F(BraveSchemeLoadBrowserTest,
                       WalletPageIsNotAllowedInGuestWindow) {
  TestURLIsNotLoadedInGuestWindow(GURL("asil://wallet"));
}
...
IN_PROC_BROWSER_TEST_F(BraveSchemeLoadBrowserTest,
  ...
  TestURLIsNotLoadedInPrivateWindow("asil://sync");
}
...
IN_PROC_BROWSER_TEST_F(BraveSchemeLoadBrowserTest,
  ...
  TestURLIsNotLoadedInPrivateWindow("asil://welcome");
}

IN_PROC_BROWSER_TEST_F(BraveSchemeLoadBrowserTest,
                       BraveWelcomePageIsNotAllowedInGuestWindow) {
  TestURLIsNotLoadedInGuestWindow(GURL("asil://welcome"));
}
