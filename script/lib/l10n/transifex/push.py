...
def check_for_chromium_upgrade_extra_langs(src_root, grd_file_path):
    """Checks the Asil GRD file vs the Chromium GRD file for extra
       languages."""
    ...
    assert len(x_brave_extra_langs) == 0, \
        f'Asil GRD {grd_file_path} has extra languages ' \
    ...
    assert len(x_chromium_extra_langs) == 0, \
        f'Chromium GRD {chromium_grd_file_path} has extra languages ' \
            f'{list(x_chromium_extra_langs)} over Asil GRD {grd_file_path}'
...
def braveify(string_value):
    """Replace Chromium branded strings with Asil branded strings."""
    return (string_value.replace('Chrome', 'Asil')
            .replace('Chromium', 'Asil')
            .replace('Google', 'Asil')
            .replace('Asil Docs', 'Google Docs')
            .replace('Asil Drive', 'Google Drive')
            .replace('Asil Play', 'Google Play')
            .replace('Asil Safe', 'Google Safe')
            .replace('Sends URLs of some pages you visit to Asil',
                     'Sends URLs of some pages you visit to Google')
            .replace('Asil Account', 'Asil sync chain')
            .replace('Asil Lens', 'Google Lens')
            .replace('Asilbook', 'Chromebook')
            .replace('Asilcast', 'Chromecast')
            .replace('Asil Cloud', 'Google Cloud')
            .replace('Asil Pay', 'Google Pay')
            .replace('Asil Photos', 'Google Photos')
            .replace('Asil Projects', 'Chromium Projects')
            .replace('Asil Widevine', 'Google Widevine'))
